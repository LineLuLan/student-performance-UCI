...
|------|----------|----------------|-----------------------|-------------------------------------|
| 1 | Which signals matter? | **Key Drivers of Final Grade** (lollipop, *dual-mode toggle*) | *Pearson r* mode: top 10 of 15 numeric features by `\|r\|` (cutoff 0.10), green = protective, red = risky. *RF Importance* mode: top 10 of 13 RF input features by Gini importance, blue (non-linear, no sign). Toggle reveals e.g. `absences` jumping from rank 15 (Pearson) to rank 4 (RF). | Click toggle → switch modes; click any dot → scatter repoints to that feature; hover → value + interpretation. |
...
| 3 | Are there student types? | **Student Behavioural Profiles** (K-Means, *subject-aware*) | 3 cluster tabs (Focused / Average / Social Risk) with name, size, cohort share, average G3, at-risk badge, six attribute bars. Cluster numbers re-fit when the *Subject* toggle changes (e.g. Social Risk Group at-risk = 29.4% combined → 42.1% Math-only → 21.3% Portuguese-only). | Tab click → switches the active persona; Subject toggle → re-fits all three clusters. |
