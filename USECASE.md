...
|------|----------|----------------|-----------------------|-------------------------------------|
| 1 | Which signals matter? | **Key Drivers of Final Grade** (lollipop) | Top 10 of 15 numeric features ranked by Pearson *r* (cutoff `\|r\| ≥ 0.10`), green stems = protective, red stems = risky. *r*-value printed next to each dot. | Click any dot → scatter repoints to that feature; hover → sign, `r`, `R²` impact (%). |
